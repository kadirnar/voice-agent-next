...
Rules live in the baseline file and can be tuned there. A gated metric that the run
should have produced but did not (e.g. every turn of a condition was missed) fails as
``missing``; metrics without a baseline are listed as ``new``. The gate uses medians of
many turns and batches, so a single slow turn on a shared runner cannot fail it; the CLI
can additionally re-run failing sections and fail only on confirmed regressions.
"""
...
def gated_metrics(summary: RunSummary) -> dict[str, GatedMetric]:
    """The gated statistic of every gated metric of a run summary."""
    ...
    for key, dist in summary.metrics.items():
        ...
        out[key] = GatedMetric(
            value=value, stat=stat, unit=_unit(key), rule=rule, ci95=ci, n=dist.n
        )
    return out
...
def entry_from_run(results: RunResults) -> BaselineEntry:
    """A baseline entry holding the gated metrics of ``results``."""
...
def compare_to_baseline(
    ...
    *,
    ...
    current = gated_metrics(results.summary)
    ...
    for name, base in entry.metrics.items():
        ...
        rule = baseline.rules.get(base.rule)
        if rule is None:
