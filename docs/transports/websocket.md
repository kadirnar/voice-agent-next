...
|---|---|---|---|
| `invalid_api_key` | yes | HTTP 401 | The server requires an API key and none (or a wrong one) was sent: the upgrade is refused |
...
* `api_keys`: require one of these keys before the upgrade (see
  [API keys](#api-keys)); default: none;
* `forward_events`;
...
* Require an API key with `api_keys=[...]` (see [API keys](#api-keys)); `van serve`
  generates one when it listens beyond this machine. For other schemes, authenticate
  before the upgrade in `process_request` (a cookie, a signed URL), or in a factory with
  `transport.hello["metadata"]`: raise
  ...
  without details.
* A non-loopback `host` without `api_keys` or a `process_request` hook logs a warning:
  nothing authenticates the clients.
...
With `api_keys` (`van serve --api-key`), the upgrade needs one of the keys, else HTTP 401
with `{"type": "error", "code": "invalid_api_key"}` (after the Origin check, which answers
403 first). Clients send the key in one of these ways:

* `Authorization: Bearer <key>` or an `api-key: <key>` header (native clients, backends);
* browsers cannot set WebSocket headers: they pass the subprotocol `van-key.<key>`, and
  optionally `van-ws`, which the server then selects:

```js
const ws = new WebSocket("wss://agent.example.com/", ["van-ws", "van-key." + key]);
```

Keys are compared in constant time and never logged. Keys generated by `van serve`
(`van_...`) are valid subprotocol tokens; choose your own keys from letters, digits, `-`,
`_` and `.` so that browsers can send them. `/health`, `/ready` and `/metrics` stay open.
A key in a browser page is visible to its users: give pages short-lived keys, or put an
authenticating proxy in front for public sites.
