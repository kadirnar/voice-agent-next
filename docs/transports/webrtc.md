...
* `401`: the server has `api_keys` and the request has no valid
  `Authorization: Bearer <key>` (`POST /offer` and `GET /config`, which may carry TURN
  credentials).
* `403`: `POST /offer` or `GET /config` from a page whose `Origin` is not allowed (see
...
WebRTCAgentServer(
    ...
    max_sessions=64, forward_events=True,
    index_html=None, cors_origins=(), ssl=None, serve_http=True,
    max_session_duration=3600.0, idle_timeout=300.0,
    allowed_origins=(), api_keys=None,
    **transport_options,                     # WebRTCTransport options
)
```

* `max_session_duration` ends a session after this many seconds, and `idle_timeout` after
  this many seconds in which the peer sent neither audio nor a data-channel message (a
  connected microphone track streams audio, even silence, so it never idles). The peer
  gets an `error` message (`session_expired` or `session_idle`, `fatal: true`) on the data
  channel, then the peer connection is closed. `None` disables a limit.
* `allowed_origins` and `api_keys`: see [Security](#security-and-deployment).
...
* **Origin check.** Browsers send an `Origin` header with `POST /offer`. Without a check,
  any website a user visits could start a session (and spend its engine) from their
  browser with a simple cross-site `POST`. The signalling server answers `POST /offer`
  and `GET /config` only for clients that send no `Origin` (native clients), pages from
  this machine (`http://localhost:*`...), the server's own origin (its `index_html` page:
  `Origin` matches `Host`), and the origins in `allowed_origins` or `cors_origins`
  (`van serve --allowed-origin`). Others get 403.
* **API keys.** Each session may hold a paid engine connection. `api_keys=[...]`
  (`van serve --api-key`; generated by `van serve` beyond loopback) makes `POST /offer`
  and `GET /config` require `Authorization: Bearer <key>`:

  ```js
  const answer = await fetch(server + "offer", {
    method: "POST",
  ...
  For other schemes, mount `handle_offer` behind your auth, or check
  `transport.offer["metadata"]` in a factory (raising there ends that peer's session).
* Sessions are bounded: `max_sessions` (64), `max_session_duration` (one hour) and
  `idle_timeout` (five minutes).
* Only list trusted origins in `cors_origins` and `allowed_origins`.
