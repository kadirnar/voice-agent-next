...
* **Tools:** `ResponseToolCall` → tools run concurrently (`execute_function_call`, timeouts, errors become tool outputs) → outputs are sent back once the response is done; only the last output triggers the follow-up response; `max_tool_steps` bounds loops. Tool calls withdrawn by the engine (`ToolCallCancelled`) are cancelled. A slow blocking round gets one watchdog filler (`tool_filler_delay`); `function_tool(blocking=False)` tools report back later (native scheduling via `send_async_tool_output`, else an acknowledgement output + a context message when idle); `ToolContext.report_progress` and `session.delegate()` complete the picture. See `docs/concepts/tools.md`.
...
* **Omni models:** an LLM with `capabilities.audio_output` (LFM2.5-Audio, `gpt-audio`, Qwen-Omni...) streams its own speech in `ChatChunk.audio`; without a TTS (or with `use_llm_audio`) the cascade plays it as `ResponseAudio`, with the text as the transcript. With `stt=None` as well, only VAD + turn detection remain: native speech-to-speech on the cascade runtime (see `docs/concepts/omni-models.md`).
