...
* **Barge-in:** `InputSpeechStarted` while a response is generating or playing → clear transport audio, drop queued frames, `interrupt(item_id, played_ms)`, trim the history message to what was heard and mark it `interrupted`.
