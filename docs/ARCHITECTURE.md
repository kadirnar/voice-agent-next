...
* **Handoffs:** a tool that returns another `Agent` (or a `Handoff`) switches the active agent once its round is over: old `on_exit` → history carry-over (`full`/`summary`/`none`/custom) → `update()` of instructions and tools, then `update_chat_ctx()`/`update_voice()` where the engine supports them → `agent_handoff` → new `on_enter` → greeting or reply. `session.userdata` is shared by all agents; `Flow` builds step-by-step conversations on top. See [handoffs and flows](concepts/handoffs.md).
