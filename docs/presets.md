...
|---|---|---|---|
| `local-gpu` | local | faster-whisper `large-v3-turbo` (CUDA) · Ollama `qwen3.5:4b` (thinking off) · Kokoro v1.0 fp16 · Silero · Smart Turn | Linux/Windows, an NVIDIA GPU usable by CTranslate2, Ollama |
| `apple` | local | MLX: Parakeet TDT 0.6B v3 (parakeet-mlx) · mlx-lm `Qwen3.5-4B-4bit` → Ollama `qwen3.5:4b` (thinking off) · Kokoro (mlx-audio) · Silero · Smart Turn | Apple silicon with an arm64 Python, the `mlx` extra, `mlx_lm.server` or Ollama |
| `local-cpu` | local | sherpa-onnx `zipformer-en-kroko` · Ollama `LiquidAI/lfm2.5-1.2b-instruct` · Kokoro v1.0 fp16 · Silero · Smart Turn + `lm_turn` (`fused`) | any OS, Ollama |
...
* **local-cpu.** On a Ryzen 5 5600 with no GPU, the streaming Kroko Zipformer
  ...
  ([sherpa-onnx.md](providers/sherpa-onnx.md#latency)). Kokoro v1.0 fp16 is the measured
  TTS: the first clause takes about 400 ms on this CPU.
* **LLM of `local-cpu`: a trade-off** ([measurements](benchmarks/results.md#local-defaults-turn-detection-and-llm-2026-09-25-155)).
  LFM2.5 1.2B stays the default because it is fast on a CPU alone: about 220 ms to the
  first token and 40 tokens/s on the Ryzen 5 5600, against 950 ms and 10 tokens/s for
  Qwen3.5-4B. It leaves the CPU to STT and TTS; note 03 names CPU contention as the main
  risk of this profile. It is weak at reasoning and tools: 10 % on T5 (Big Bench Audio)
  and 27 % pass@1 on T6, against 35 % and 64 % for Qwen3.5-4B. If Ollama has a GPU, or
  the agent calls tools, use Qwen3.5-4B with thinking off:
  `--llm '{provider: ollama/qwen3.5:4b, reasoning_effort: none}'`. It costs about
  370 ms of voice-to-voice latency (T1 p50 872 → 1,240 ms with the LLM on the GPU),
  because its longer first clauses keep Kokoro busy longer.
* **Fused turn detection in `local-cpu`.** Smart Turn v3.2 is fused with the `lm_turn`
  text model (SmolLM2-135M, 137 MB, extra `text-turn`; [lm-turn.md](providers/lm-turn.md)).
  With a streaming STT the transcript is ready at the pause, so the text half adds about
  30 ms of end-of-turn delay and nothing to v2v (T1 p50 928 vs 927 ms). It ends fewer
  turns too early: on eot-bench English 8.8 % false cut-offs instead of 10.8 %, at a
  53 ms lower mean latency; in the T4 battery 28 % premature replies instead of 36 %
  (3 runs each). Cost: 32 MB of Python packages (`tokenizers`, `huggingface-hub`) and a
  137 MB model. `local-gpu` keeps Smart Turn alone: behind faster-whisper (a batch STT)
  the text half waits for the final transcript, which added 100–230 ms of end-of-turn
  delay with no fewer premature replies. `apple` keeps Smart Turn alone until it is
  measured on a Mac. Use `turn_detector: smart_turn` to go back, or
  `turn_detector: {provider: fused, audio: smart_turn, text: lm_turn}` to opt in.
...
* **local-gpu.** On an RTX 5070 Ti with CUDA float16, faster-whisper's final transcript
  ...
  speed. The LLM is Qwen3.5-4B with thinking off (`reasoning_effort: none`), chosen for
  quality: 64 % pass@1 on T6 and 35 % on T5, against 27 % and 10 % for LFM2.5 1.2B.
  Voice-to-voice latency does not stay under 1 s on this stack: T1 p50 1,672 ms against
  1,101 ms with LFM2.5 1.2B. Most of the difference is Kokoro, still on the CPU, rendering
  Qwen's longer first clauses (the LLM's first token takes about 60 ms on the GPU).
  LFM2.5-2.6B reasons before every answer and cannot be told not to (T1 p50 2,656 ms).
  For note 03's 16 GB pick use
  `--llm '{provider: ollama/qwen3.5:9b, reasoning_effort: none}'` (not measured). Ollama
  lets Qwen3.5 think before every answer unless `reasoning_effort: none` is set; `van
  presets` notes it when an override leaves thinking on. The preset fails its check when
...
* **apple.** Everything on the GPU through MLX ([mlx.md](providers/mlx.md)): Parakeet TDT
  ...
  when the turn ends, `mlx_lm.server` runs Qwen3.5-4B at 4-bit (Ollama's `qwen3.5:4b` with
  thinking off is the failover when no mlx-lm server runs), and Kokoro-82M runs through mlx-audio. The check says how to
...
| a `fused` turn detector: its audio and text halves, like components (`lm_turn` needs `tokenizers`) | the `text-turn` extra |
| an Ollama model that thinks before answering (Qwen3, LFM2.5-Thinking, LFM2.5-2.6B, DeepSeek-R1) without `reasoning_effort` | a note, not a problem: `{provider: ollama/<model>, reasoning_effort: none}` |
...
Turn 9 MB, `lm_turn` SmolLM2-135M 137 MB, faster-whisper `large-v3-turbo` about 1.6 GB.
...
```console
$ van run --preset local-cpu --llm '{provider: ollama/qwen3.5:4b, reasoning_effort: none}' \
    --instructions "You are a barista."
...
```yaml
extends: local-cpu
llm: {provider: ollama/qwen3.5:4b, reasoning_effort: none}   # replaces the preset's LLM
...
# or in one step
session, agent = session_from_preset(
    "local-cpu", llm={"provider": "ollama/qwen3.5:4b", "reasoning_effort": "none"}
)
