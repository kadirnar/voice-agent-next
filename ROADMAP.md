...
- **Wave 5 (done): hardening from a full codebase audit.** Telephony auth and call-ID validation (#133, P0), OpenAI key scoping (#134), failure-safe session lifecycle (#135), cascade robustness (#136), exact TTS usage + an overhead gate with an absolute floor (#137), resampler/PCM correctness (#138), secure-by-default serving (#139), shared provider helpers and consistent option names (#140), config layering/offline/exports/hot paths (#141), GC-safe streams (#150, #151) and failure-path tests (#142).
...
- **Wave 6 (done): final follow-ups.** Evidence-based local defaults (#155: fused turn detection in `local-cpu`, qwen3.5:4b in `local-gpu`), layered `van serve` flags + token auth for every protocol + carrier verification (#156), resampler drains / STT reconnect / usage attribution (#157), benchmark gate floors + LLM-driven T6 caller + a T1–T7 summary (#158), a shared multilingual Whisper hallucination guard (#159).
- **Owner actions (deferred by choice):** enable GitHub Pages (`PAGES_ENABLED`) and the first PyPI release (`docs/releasing.md`).
