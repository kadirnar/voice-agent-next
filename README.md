...
| | Local | Cloud |
|---|---|---|
| **Speech-to-speech engines** | Moshi / PersonaPlex (full duplex), omni LLMs (LFM2.5-Audio), any OpenAI-Realtime-compatible server: Speaches, LocalAI, vLLM-Omni | OpenAI Realtime & GPT-Live, Gemini Live, Amazon Nova 2 Sonic, Azure OpenAI Realtime, xAI Grok Voice, Qwen-Omni Realtime |
| **STT** | sherpa-onnx (streaming Zipformer/NeMo, Parakeet, Moonshine, SenseVoice, Whisper), Moonshine Streaming, NeMo-Speech.cpp (Nemotron), faster-whisper (CPU / CUDA), MLX (Parakeet, Whisper) | Deepgram Nova-3 & Flux, AssemblyAI Universal-Streaming, Soniox, Speechmatics, ElevenLabs Scribe v2, OpenAI transcribe, Cartesia Ink |
...
Every roadmap milestone has landed; see the [roadmap](ROADMAP.md) for what's next.
